...
1. transport layer 
		클라가 접속하려는 서버가 mitm 공격을 당한건 아닌지(바꿔치기 당한서버가 아니고 처음 접속한 서버가 맞는지) 를 확인하기 위한 host 공개키 인증을 수행하고, 암호화채널을 만든다.
...
클라에서 요청보내면 22 번 포트의 tcp soket 이 받고 sshd 한테 넘겨줌. 그럼 sshd 가 pty 또는 실행 프로세스 한테 stdin 으로 넘겨주고, 결과 데이터를 클라의 임시포트의 tcp soket 한테 보냄.
