...
즉 클라이언트가 배포된 URL 이다. 클라이언트에 접속하면 JS 파일이 


브라우저가 강제하는 기술이고, post 맨에서 바로 서버 url 로 요청보내거나 브라우저에서 바로 서버 url 로 요청보내는것은 다른 페이지에서 js 파일이 요청보내는것이 아니기 때문에 cors 대상이 아니다.