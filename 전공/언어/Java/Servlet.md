...
# Servlet 생명주기
요청이 들어오면 해당하는 요청에 대한 기존에 생성된 서블릿이 있는지 메모리에서 확인하고 없으면 init() 딱 한번으로 메모리에 적제한다. 그다음 서블릿의 service() 를 호출한다. service() 를 호출하고 http 메서드를 보고 doPost(), doGet() 등을 호출한다. 서블릿이 더이상 필요 없어지면 distory() 를 호출한다. 생성, 삭제시 필요한 작업이 있다면 init(), distory() 를 오버라이딩 하면 됨.
